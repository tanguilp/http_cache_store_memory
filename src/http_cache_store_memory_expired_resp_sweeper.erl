%% @private
-module(http_cache_store_memory_expired_resp_sweeper).
...
handle_info(sweep, State) ->
    telemetry:span([http_cache_store_memory, expired_resp_sweeper],
                   #{},
                   fun() ->
                      sweep(),
                      {ok, #{}}
...
do_sweep(ObjectKey) ->
    ...
    case ets:lookup(?OBJECT_TABLE, ObjectKey) of
        [{_, _, _, _, #{grace := Expires}, _}] when Expires =< Now ->
            http_cache_store_memory:delete_object(ObjectKey, expired);
        _ ->
            ok
    end,
