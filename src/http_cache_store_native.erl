...
put_no_broadcast(RequestKey, UrlDigest, VaryHeaders, Response, RespMetadata, AlternateKeys) ->
  put(RequestKey, UrlDigest, VaryHeaders, Response, RespMetadata, AlternateKeys, false).

put(RequestKey, UrlDigest, VaryHeaders, Response, RespMetadata, AlternateKeys, DoBroadcast) ->
  case http_cache_store_native_stats:is_limit_reached() of
    true ->
      {error, store_overloaded};

    false ->
      do_put(RequestKey, UrlDigest, VaryHeaders, Response, RespMetadata, AlternateKeys, DoBroadcast)
  end.
    ...
    [{_, _, _, _, _, _, ObjectAltKeys, _}] ->
      case lists:any(fun(AltKey) -> lists:member(AltKey, ObjectAltKeys) end, AltKeys) of
        true ->
          ...
          do_invalidate_by_alternate_key(AltKeys, ets:next(?OBJECT_TABLE, ObjectKey), NbDeleted + 1);
...
unix_now() -> os:system_time(second).
