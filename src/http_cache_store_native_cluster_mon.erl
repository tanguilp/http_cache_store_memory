...
-export([
         ...
         broadcast_cached_object/2,
         request_cached_object/2,
         send_cached_object/2
        ]).
-export([start_link/0, init/1, handle_call/3, handle_continue/2, handle_cast/2, handle_info/2]).
...
handle_continue(_, State) ->
  NbObjects = application:get_env(application:get_application(), warmup_nb_objects, ?WARMUP_NB_OBJECTS),
...
handle_cast({_, _} = Cmd, State) ->
  case is_priority_command(Cmd) of
    ...
    false ->
      ...
      if
        NbRunningCommands < ?RUNNING_COMMAND_LIMIT ->
          http_cache_store_native_cluster_sup:start_worker(Cmd);
