...
start_link({invalidate_url, UrlDigest}) ->
  Pid = erlang:spawn_link(http_cache_store_native, invalidate_url_no_broadcast, [UrlDigest]),
  {ok, Pid};
start_link({invalidate_by_alternate_key, AltKeys}) ->
  Pid = erlang:spawn_link(http_cache_store_native, invalidate_by_alternate_key_no_broadcast, [AltKeys]),
  {ok, Pid};
...
warmup_node(Node, {_, ObjectKey, SeqNumber} = LRUKey, NbObjects) ->
  case ets:lookup(?OBJECT_TABLE, ObjectKey) of
    [{{RequestKey, _}, VaryHeaders, UrlDigest, Response, RespMetadata, _, AltKeys, SeqNumber}] ->
