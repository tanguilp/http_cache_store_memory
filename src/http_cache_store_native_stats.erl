...
allocated_memory_used() ->
  MemoryLimit = application:get_env(application:get_application(?MODULE), memory_limit, ?DEFAULT_LIMIT),
...
system_memory_use() ->
  system_memory_use(maps:from_list(memsup:get_system_memory_data())).
...
init(_) ->
  ...
  schedule_collect(),
  {ok, #{total_mem => 0, objects_mem => 0, objects_count => 0, lru_mem => 0}}.
...
handle_info(collect_stats, _Stats) ->
  ...
  telemetry:execute(
   [http_cache_store_native, memory],
   Stats,
   #{}
   ),
  schedule_collect(),
...
collect_interval() ->
  application:get_env(application:get_application(), pull_table_stats_interval, ?DEFAULT_INTERVAL).
